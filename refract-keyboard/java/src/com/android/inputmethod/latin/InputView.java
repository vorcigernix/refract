...
public final class InputView extends FrameLayout {
    ...
    /**
     * Keeps the bottom key row clear of the system navigation area.
     *
     * <p>The IME window spans the full display height, so with gesture navigation the
     * gesture pill sits on top of the last key row. Worse, while an IME is visible the
     * system also draws a hide-keyboard chevron there, plus an IME-switcher button
     * when more than one input method is enabled. Those are SystemUI affordances
     * drawn above this window, so they cannot be themed and they overlap real keys.
     *
     * <p>AOSP predates gesture navigation: its only bottom spacing is
     * {@code keyboardBottomPadding}, a fraction of keyboard height that knows nothing
     * about insets. Padding by the measured navigation-bar inset instead means no
     * space is wasted on devices that report no inset.
     *
     * <p>The padding goes on the visible panels, NOT on this view. This view fills the
     * whole full-height IME window and paints nothing; padding it would leave the
     * resulting strip outside any background, so the host app would show through
     * beneath the keys. The panels wrap their content and do paint a background, so
     * padding them keeps the strip filled with the board colour.
     */
    @Override
    public WindowInsets onApplyWindowInsets(final WindowInsets insets) {
        final int navigationBarBottom =
                insets.getInsets(WindowInsets.Type.navigationBars()).bottom;
        applyBottomInset(findViewById(R.id.main_keyboard_frame), navigationBarBottom);
        applyBottomInset(findViewById(R.id.emoji_palettes_view), navigationBarBottom);
        // Returned unconsumed: children may still want the horizontal gesture insets.
        return insets;
    }

    private static void applyBottomInset(final View view, final int bottomInset) {
        if (view == null || view.getPaddingBottom() == bottomInset) {
            return;
        }
