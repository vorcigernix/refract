...
public final class KeyboardSwitcher implements KeyboardState.SwitchActions {
    ...
    /**
     * Night-mode flag baked into {@link #mThemeContext}, or -1 before the first build.
     *
     * <p>Needed because KeyboardTheme.Refract is a single DayNight theme. The legacy
     * themes made light and dark separate theme ids, so a system theme switch changed
     * the KeyboardTheme instance and invalidated the cached context. With one DayNight
     * theme the instance never changes, and the Resources comparison below can also
     * hold, so without this check the keyboard keeps rendering its stale palette while
     * the rest of the app follows the system.
     */
    private int mThemeContextNightMode = -1;
    ...
    public void updateKeyboardTheme(@NonNull Context displayContext) {
        final boolean themeUpdated = updateKeyboardThemeAndContextThemeWrapper(displayContext);
    ...
    private boolean updateKeyboardThemeAndContextThemeWrapper(final Context context) {
        ...
        if (mThemeContext == null || nightMode != mThemeContextNightMode
                || !mThemeContext.getResources().equals(context.getResources())) {
            mThemeContextNightMode = nightMode;
            mThemeContext = createThemeContext(context);
    ...
    /**
     * Builds the themed context the keyboard views are inflated from.
     *
     * <p>The key drawables and text colours in KeyboardTheme.Refract are all
     * {@code ?attr/color*} references, so they only resolve if the M3 colour roles
     * are present on this context's theme. Two steps, and the order matters:
     *
     * <ol>
     *   <li>{@link ContextThemeWrapper} installs KeyboardTheme.Refract as the
     *       <em>base</em> theme, bringing in its Theme.Material3Expressive.DayNight
     *       parent and therefore the colour roles.
     *   <li>The single-argument {@code wrapContextIfAvailable} then applies the
     *       dynamic-colour <em>overlay</em> on top, replacing those roles with the
     *       wallpaper-derived palette.
     * </ol>
     *
     * <p>Do not collapse this into {@code wrapContextIfAvailable(context, styleId)}:
     * that overload treats its second argument as a theme <em>overlay</em>, not a
     * base theme, so KeyboardTheme.Refract's Material parent is never installed.
     * The {@code ?attr/color*} lookups then resolve to 0, and because
     * {@code KeyDrawParams.selectColor} treats 0 as "unset" and KeyboardView never
     * seeds a text colour, every key label renders fully transparent.
     *
     * <p>The overlay step is a no-op where no dynamic palette exists, leaving the
     * static DayNight colours in place.
     */
    private static Context createThemeContext(final Context context) {
        final Context themedContext =
                new ContextThemeWrapper(context, R.style.KeyboardTheme_Refract);
